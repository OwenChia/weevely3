        ...
        if isinstance(ip, BaseIP):
            if ips and ips[-1]._version != ip._version:
                raise IPTypeError(
                    ...
                    ' v%d: %s, v%d %s' % (ips[-1]._version, str(ips[-1]),
                                          ip._version, str(ip)))
            ips.append(ip)
    ...
    def __contains__(self, other):
        ...
        if isinstance(other, BaseNet):
            return (int(self.network) <= int(other._ip)
                    and int(self.broadcast) >= int(other.broadcast))
...
class BaseV4(object):
    ...
    def is_private(self):
        ...
        """
        return (self in IPv4Network('10.0.0.0/8')
        ...
        if len(addr) == 2:
            ...
            if len(mask) == 4:
                ...
                if self._is_hostmask(addr[1]):
                    ...
                    self.netmask = IPv4Address(
                        self._ip_int_from_string(addr[1]))
                ...
                self.netmask = IPv4Address(
                    self._ip_int_from_prefix(self._prefixlen))
        else:
            self._prefixlen = 32
            self.netmask = IPv4Address(
                self._ip_int_from_prefix(self._prefixlen))
...
class BaseV6(object):
    ...
    def _is_valid_ip(self, ip_str):
        ...
        if ((ip_str.startswith(':') and not ip_str.startswith('::'))
                or (ip_str.endswith(':') and not ip_str.endswith('::'))):
            return False
    ...
    def is_reserved(self):
        ...
        """
        return (self in IPv6Network('::/8') or self in IPv6Network('100::/8')
