...
class Console(Module):
    ...
    def _query(self, vector, args):
        ...
        if errsep not in result:
            ...
            return {
                ...
                'result': [
                    line.split(linsep)
                    for line in result.replace(linsep +
                                               colsep, colsep).split(colsep)
                    if line
                ]
            }
    ...
    def run(self):
        ...
        if vector == 'pgsql' and database:
            ...
            vector += ('' if self.args.get('user') and self.args.get('passwd')
                       else '_fallback')
        ...
        # Check credentials
        self.args['query'] = ('SELECT USER;' if vector.startswith('pgsql') else
                              'SELECT USER();')
