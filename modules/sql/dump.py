...
class Dump(Module):
    ...
    def init(self):
        ...
        self.register_arguments([
            ...
            {
                'name': '-lpath',
                'help': 'Dump to local path (default: temporary file)'
            },
            {
                'name': '-vector',
                'choices': self.vectors.get_names(),
                'default': 'mysqldump_php'
            }
        ])
    ...
    def run(self):
        ...
        if not lpath:
            temp_file = tempfile.NamedTemporaryFile(
                ...
                (self.args['user'], self.args['passwd'], self.args['host'],
                 self.args['db']),
                delete=False)
        ...
        if not os.path.isabs(lpath):
            if lpath.startswith('~'):
                lpath = os.path.expanduser('~') + os.path.join(
                    '/', lpath.replace('~/', ''))
            else:
