...
class Su(Module):
    ...
    def init(self):
        ...
        self.register_vectors([
            ShellCmd(
                ...
                name="sh_expect",
                postprocess=
                lambda x: re.findall('Password: (?:\r\n)?([\s\S]+)', x)[0] if 'Password: ' in x else ''
            ),
            ShellCmd(
                """python -c 'import pexpect as p,sys;c=p.spawn("su ${user} -c ${command}");c.expect(".*assword:");c.sendline("${ passwd }");i=c.expect([p.EOF,p.TIMEOUT]);sys.stdout.write(c.before[3:] if i!=p.TIMEOUT else "")'""",
                "pyexpect")
        ])
        ...
        self.register_arguments(
            [{
             ...
             {
                 'name': '-user',
                 'help': 'User to run the command with',
                 'default': 'root'
             }, {
                 'name': '-stderr_redirection',
                 'default': ' 2>&1'
             },
             {
                 'name':
                 '-vector-sh',
                 'choices': ('system', 'passthru', 'shell_exec', 'exec',
                             'popen', 'proc_open', 'perl_system', 'pcntl')
             }, {
                 'name': '-vector',
    ...
    def run(self):
        ...
        return self.vectors.get_result(
            name=self.args['vector'], format_args=format_args)
