...
class Filesystem(Module):
    ...
    def check_writable_binaries(self):
        ...
        for path in [
                ...
                '/usr/local/sbin', '/lib/', '/usr/lib/', '/usr/local/lib'
        ]:
    ...
    def check_home_writable(self):
        ...
        result = ModuleExec("file_find",
                            ['-no-recursion', '-writable', '/home/']).run()
    ...
    def check_home_executable(self):
        ...
        result = ModuleExec("file_find",
                            ['-no-recursion', '-executable', '/home/']).run()
    ...
    def check_readable_etc(self):
        ...
        files_paths = [
            'shadow', 'ap-secrets', 'mysql/debian.cnf', 'sa_key$', 'keys',
            r'\.gpg', 'sudoers'
        ]
    ...
    def check_readable_logs(self):
        ...
        readable_files = ModuleExec("file_find",
                                    ['-readable', '/var/log/']).run()

        files_paths = [
            'lastlog', 'dpkg', 'Xorg', 'wtmp', 'pm', 'alternatives', 'udev',
            'boot'
        ]

        return [
            f for f in readable_files
            if f and not f.endswith('gz') and not f.endswith('old') and any(
