...
class Phpconf(Module):
    ...
    def _check_user(self):
        user = ModuleExec('system_info',
                          ['-info', 'whoami']).load_result_or_run('whoami')
        if not user:
    ...
    def _check_functions(self):
        ...
        for ftype, flist in functions.items():
            ...
            func_found = PhpCode(
                """foreach ( Array("${ '", "'.join(functions) }") as $f) if(function_exists($f)&&is_callable($f)) print($f. "\n");"""
            ).run({
                'functions': flist
            })
    ...
    def run(self):
        ...
        self.os_type = ModuleExec('system_info',
                                  ['-info', 'os']).load_result_or_run('os')
        self.php_version = ModuleExec(
            'system_info',
            ['-info', 'php_version']).load_result_or_run('php_version')
        ...
        results = [('Operating System',
                   ...
                   ] + self._check_features() + self._check_classes(
