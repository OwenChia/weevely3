...
class Scan(Module):
    ...
    def run(self):
        ...
        for ips_chunk in utils.strings.chunks(
                IPs, self.args['addresses_per_request']):
            for prts_chunk in utils.strings.chunks(
                    prts, self.args['ports_per_request']):
        ...
        for result_string in results_string.split('\n'):
            ...
            if addr_string_splitted[0] == 'OPN':
                ...
                address = addr_string_splitted[1]
                error = '%s (%s)' % (' '.join(addr_string_splitted[2:-1]),
