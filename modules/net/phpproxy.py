...
class Phpproxy(Module):
        ...
        self.register_arguments([
            {
                ...
                'rpath',
                'help':
                'Remote path where to install the PHP proxy script. If it is a folder find the first writable folder in it',
                'default':
                '.',
                'nargs':
                '?'
            },
            {
                ...
                '-rname',
                'help':
                'Set a specific file name ending with \'.php\'. Default is random',
                'default':
                '%s.php' % utils.strings.randstr(6)
            },
            {
                'name': '-no-autoremove',
                'action': 'store_true',
    ...
    def run(self):
        ...
        result = ModuleExec('file_upload2web', [
            os.path.join(self.folder, 'poxy.php'), self.args['rpath'],
            self.args['rname']
        ]).run(self.args)

        if not (result and len(result[0]) == 2 and result[0][0]
                and result[0][1]):
            return
