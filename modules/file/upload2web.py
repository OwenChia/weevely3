...
class Upload2web(Module):
        ...
        self.register_arguments([
            {
                ...
                'lpath',
                'help':
                'Local file path. Set remote file name when used with -content.'
            },
            {
                'name':
                'rpath',
            ...
            },
            {
                'name': '-simulate',
                'help':
                'Just return the positions without uploading any content',
                'action': 'store_true',
                'default': False
            },
        ])
    ...
    def _get_env_info(self, script_url):
        ...
        for pieceurl, piecefolder in zip(
                reversed(url_folder_pieces), reversed(folder_pieces)):
        ...
        self.base_folder_url = urllib.parse.urlunsplit(
            script_url_splitted[:2] + (base_url_path_folder, ) +
    ...
    def run(self):
        ...
        if self.args.get('simulate') or ModuleExec("file_upload",
                                                   file_upload_args).run():
