...
class Curl(BaseTest):
    ...
    def test_sent_data(self):
        ...
        for vect in self.vector_list:
            ...
            self.assertEqual(
                '200 OK',
                self._headers_result([url, '-X', 'PUT', '-vector',
                                      vect])[0][-6:])
            ...
            self.assertEqual(
                'value',
                self._json_result(
                    [url, '-H', 'X-Arbitrary-Header: value', '-vector',
                     vect])['headers']['X-Arbitrary-Header'])
            ...
            self.assertEqual(
                ...
                self._json_result([
                    url, '-b', 'C1=bogus;C2=bogus2', '-vector',
                    vect
                ])['cookies'])
            ...
            result = self._json_result(
                [url, '--data', 'f1=data1&f2=data2', '-vector', vect])
            self.assertEqual({'f1': 'data1', 'f2': 'data2'}, result['form'])
            ...
            url = self.url + '/post'
            result = self._json_result(
                [url, '--data', 'FIELD=D\x41\x54A\x00B', '-vector', vect])
            self.assertEqual({'FIELD': 'DATA\x00B'}, result['form'])
    ...
    def test_unreachable(self, log_captured):
        ...
        for vect in self.vector_list:
            ...
            self.assertIsNone(
                self.run_argv(['http://co.uk', '-vector', vect])[0])
    ...
    def test_filtered(self, log_captured):
        ...
        for vect in self.vector_list:
            ...
            self.assertIsNone(
                self.run_argv([
                    'http://www.google.com:9999', '-vector', vect,
                    '--connect-timeout', '1'
                ])[0])
            self.assertEqual(messages.module_net_curl.unexpected_response,
                             log_captured.records[-1].msg)
        ...
        self.assertIsNone(
            self.run_argv(
                ['http://www.google.com:9999', '--connect-timeout', '1'])[0])
        self.assertEqual(messages.module_net_curl.unexpected_response,
                         log_captured.records[-1].msg)
    ...
    def test_closed(self, log_captured):
        ...
        for vect in self.vector_list:
            ...
            self.assertIsNone(
                self.run_argv([
                    'http://localhost:43907', '-vector', vect,
                    '--connect-timeout', '1'
                ])[0])
            self.assertEqual(messages.module_net_curl.unexpected_response,
                             log_captured.records[-1].msg)
        ...
        self.assertIsNone(
            self.run_argv(['http://localhost:19999', '--connect-timeout',
                           '1'])[0])
        self.assertEqual(messages.module_net_curl.unexpected_response,
                         log_captured.records[-1].msg)
    ...
    def test_output_remote(self):
        ...
        for vect in self.vector_list:
            ...
            result, headers, saved = self.run_argv(
                [url, '-vector', vect, '-o',
                 'test_net_curl/test_%s' % vect])
            self.assertTrue(saved)

        result, headers, saved = self.run_argv(
            [url, '-o', 'test_net_curl/test_all'])
        self.assertTrue(saved)
    ...
    def test_output_local(self):
        ...
        for vect in self.vector_list:
            ...
            result, headers, saved = self.run_argv([
                self.url + '/post', '-vector', vect, '--data', 'FIND=THIS',
                '-o', temp_file.name, '-local'
            ])
            self.assertTrue(saved)
        ...
        # Check saved = False with a wrong path
        result, headers, saved = self.run_argv(
            [self.url, '-o', 'bogus/bogusbogus', '-local'])
        self.assertFalse(saved)
