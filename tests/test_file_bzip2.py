...
chown www-data: -R "$BASE_FOLDER/"
    ...
    def test_compress_decompress(self):
        ...
        self.assertEqual(
            subprocess.check_output(
                'cat "%s"' % self.uncompressed[0], shell=True),
            self.binstring[0])
        ...
        subprocess.check_call(
            'stat -c %%a "%s"' % self.uncompressed[0], shell=True)
    ...
    def test_compress_decompress_multiple(self):
        ...
        for index in range(0, len(self.compressed)):
            ...
            # Decompress and check test file
            self.assertTrue(
                self.run_argv(["--decompress", self.compressed[index]]))
            self.assertEqual(
                subprocess.check_output(
                    'cat "%s"' % self.uncompressed[index], shell=True),
    ...
    def test_already_exists(self, log_captured):
        ...
        self.assertTrue(
            self.run_argv(["--decompress", self.compressed[0], '--keep']))
        self.assertEqual(
            subprocess.check_output(
                'cat "%s"' % self.uncompressed[0], shell=True),
            self.binstring[0])
        ...
        self.assertEqual(
            ...
            "File '%s' already exists, skipping decompressing" %
            self.uncompressed[0])
        ...
        self.assertEqual(
            log_captured.records[-1].msg,
            "File '%s' already exists, skipping compressing" %
            self.compressed[0])
    ...
    def test_wrong_ext(self, log_captured):
        ...
        self.assertIsNone(
            self.run_argv(["--decompress", self.uncompressed[0]]))
