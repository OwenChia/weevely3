...
class FileDownload(BaseTest):
    ...
    def test_download_php(self):
        ...
        # Simple download
        self.assertEqual(
            self.run_argv(['test_file_download/ok.test', temp_file.name]),
            b'OK')
        with open(temp_file.name, 'r') as fd:
            self.assertEqual(fd.read(), 'OK')
        temp_file.truncate()
        ...
        self.assertEqual(
            self.run_argv(['test_file_download/ko.test', temp_file.name]),
            None)
        ...
        self.assertEqual(
            self.run_argv(['test_file_download/ok.test', '/tmp/bogus/bogus']),
            None)
        with open(temp_file.name, 'r') as fd:
            self.assertEqual(fd.read(), '')
    ...
    def test_download_sh(self):
        ...
        self.assertEqual(
            self.run_argv([
                '-vector', 'base64', 'test_file_download/ok.test',
                temp_file.name
            ]), b'OK')
        with open(temp_file.name, 'r') as fd:
            self.assertEqual(fd.read(), 'OK')
        ...
        self.assertEqual(
            self.run_argv([
                '-vector', 'base64', 'test_file_download/ko.test',
                temp_file.name
            ]), None)
        self.assertEqual(open(temp_file.name, 'r').read(), '')
        ...
        # Download of an remote unexistant file
        self.assertEqual(
            self.run_argv(['-vector', 'base64', 'bogus', temp_file.name]),
            None)
        self.assertEqual(open(temp_file.name, 'r').read(), '')
        ...
        self.assertEqual(
            self.run_argv(
                ['-vector', 'base64', 'test_file_download/ok.test', '/tmp/']),
            None)
        self.assertEqual(open(temp_file.name, 'r').read(), '')
