...
class UploadWeb(BaseTest):
    ...
    def test_file_uploadweb(self):
        ...
        self.assertEqual(
            self.run_argv([temp_file.name, rpath]), [(rpath, rurl)])
        temp_file.close()
    ...
    def test_file_uploadweb_content(self):
        ...
        self.assertEqual(
            self.run_argv(
                [temp_file_name, self.folders_rel[1], '-content', '1']),
            [(rpath, rurl)])
    ...
    def test_uploadweb_errs(self, log_captured):
        ...
        self.assertIsNone(
            self.run_argv(
                [temp_file.name, self.folders_rel[0] + '/bogus/bogus']))
        ...
        temp_file = tempfile.NamedTemporaryFile()
        self.assertIsNone(
            self.run_argv([temp_file.name, self.folders_rel[3] + '/bogus']))
        self.assertEqual(log_captured.records[-1].msg,
                         messages.module_file_upload.failed_upload_file)
