...
class Proxy(BaseTest):
    ...
    def _headers_result(self, args):
        return self.run_argv(
            args, unquoted_args='-sSL -D - -o /dev/null').splitlines()
    ...
    def test_all(self):
        ...
        # Simple GET
        url = self.url + '/get'
        self.assertEqual(url, self._json_result([url])['url'])

        # PUT request
        url = self.url + '/put'
        self.assertEqual(url, self._json_result([url, '-X', 'PUT'])['url'])
        ...
        self.assertEqual(
            'value',
            self._json_result([url, '-H', 'X-Arbitrary-Header: value'
                               ])['headers']['X-Arbitrary-Header'])
        ...
        self.assertEqual({
            ...
            'C2': 'bogus2'
                         ...
                         self._json_result([url, '-b',
                                            'C1=bogus;C2=bogus2'])['cookies'])
        ...
        result = self._json_result(
            [url],
            unquoted_args="--data FIELD=$(env echo -ne 'D\\x41\\x54A\\x00B')")
        self.assertEqual({'FIELD': 'DATAB'}, result['form'])
        ...
        self.assertIn('google', self.run_argv(['https://' + google_ip, "-k"]))

        # UNREACHABLE
        self.assertIn('Message: Bad Gateway.',
                      self.run_argv(['http://co.uk:0']))

        # FILTERED
        self.assertIn(
            'Message: Bad Gateway.',
            self.run_argv(
                ['http://www.google.com:9999', '--connect-timeout', '1']))

        # CLOSED
        self.assertIn(
            'Message: Bad Gateway.',
            self.run_argv(['http://localhost:9999', '--connect-timeout', '1']))
