...
class FileTar(BaseTest):
    ...
    folders_abs = [os.path.join(config.base_folder, f) for f in folders_rel]
    ...
    files_abs = [os.path.join(config.base_folder, f) for f in files_rel]

    tars_rel = ['test_file_tar/test_0.tar']
    tars_abs = [os.path.join(config.base_folder, f) for f in tars_rel]
    ...
    def test_compress_decompress(self):
        ...
        # Uncompress test.tar
        self.assertTrue(
            self.run_argv(["--decompress", self.tars_rel[0],
                           'test_file_tar/']))
        for file in self.files_abs:
            self.assertEqual(
                subprocess.check_output("cat %s" % file, shell=True), b'1')
        for folder in self.folders_abs:
            subprocess.check_call('stat -c %%a "%s"' % folder, shell=True)

        # Compress it again giving starting folder
        self.assertTrue(
            self.run_argv(['test_file_tar/test_1.tar', self.folders_rel[0]]))
        self.tars_rel.append('test_file_tar/test_1.tar')
        self.tars_abs.append(
            os.path.join(config.base_folder, self.tars_rel[-1]))

        # Uncompress the new archive and recheck
        self.assertTrue(
            self.run_argv(
                ["--decompress", 'test_file_tar/test_1.tar',
                 'test_file_tar/']))
        for file in self.files_abs:
            self.assertEqual(
                subprocess.check_output("cat %s" % file, shell=True), b'1')
    ...
    def test_compress_multiple(self):
        ...
        self.assertTrue(
            self.run_argv([
                'test_file_tar/test_2.tar', self.folders_rel[0],
                self.other_file_rel
            ]))
        ...
        self.assertTrue(
            self.run_argv(
                ["--decompress", 'test_file_tar/test_2.tar',
                 'test_file_tar/']))
        for file in self.files_abs:
            self.assertEqual(
                subprocess.check_output("cat %s" % file, shell=True), b'1')
        ...
        self.assertEqual(
            subprocess.check_output(
                "cat %s" % self.other_file_abs, shell=True), b'1')
    ...
    def test_already_exists(self, log_captured):
        ...
        self.assertIsNone(
            self.run_argv(['test_file_tar/test_0.tar', self.other_file_rel]))
        self.assertEqual(
