...
class FileGrep(BaseTest):
    ...
    def test_file_grep(self):
        ...
        for vect in self.vector_list:
            ...
            self.assertEqual(
                self.run_argv(['-vector', vect, self.folders_rel[0],
                               'tring1'])[0], {
            ...
            self.assertEqual(
                self.run_argv(
                    ['-vector', vect, self.folders_rel[0], 'tring[2-9]'])[0],
                {self.files_rel[2]: ['string3']})

            # grep rpath=folder2 string -> string[3]
            self.assertEqual(
                self.run_argv(
                    ['-vector', vect, self.folders_rel[2], 'string.*'])[0],
                {self.files_rel[2]: ['string3']})
    ...
    def test_file_grep_invert(self):
        ...
        for vect in self.vector_list:
            ...
            self.assertEqual(
                self.run_argv(
                    ['-vector', vect, self.folders_rel[0], 'tring1', '-v'])[0],
                {
            ...
            self.assertEqual(
                self.run_argv(
                    ['-vector', vect, self.folders_rel[0], 'bogus', '-v'])[0],
                {
                    self.files_rel[0]: ['string1'],
                    self.files_rel[1]: ['string12'],
                    self.files_rel[2]: ['string3', 'STR33']
            ...
            self.assertEqual(
                self.run_argv(
                    ['-vector', vect, self.files_rel[2], '-v', '-case',
                     'STR'])[0], {self.files_rel[2]: ['string3']})
    ...
    def test_file_grep_output_remote(self):
        ...
        for vect in self.vector_list:
            ...
            output_path = os.path.join(
                config.base_folder, 'test_file_grep',
                'test_%s_%i' % (vect, random.randint(1, 99999)))

            # grep string3 -> []
            self.assertTrue(
                self.run_argv([
                    '-vector', vect, self.folders_rel[0], 'tring4', '-output',
                    output_path
                ])[1])
            self.assertEqual(
                subprocess.check_output(
            ...
            self.assertEqual(
                self.run_argv([
                    '-vector', vect, self.folders_rel[2], 'string.*',
                    '-output', output_path
    ...
    def test_sh_err(self, log_captured):
        ...
        # wrong rpath generate None and warning print
        self.assertEqual(
            self.run_argv(['-vector', 'grep_sh', 'bogus', 'tring4'])[0], None)
        self.assertEqual(messages.module_file_grep.failed_retrieve_info,
                         log_captured.records[-1].msg)

        # wrong regex generate None and warning print
        self.assertEqual(
            self.run_argv(['-vector', 'grep_sh', '\'', 'tring4'])[0], None)
        self.assertEqual(messages.module_file_grep.failed_retrieve_info,
                         log_captured.records[-1].msg)
